...
const addService = function (userData, payloadData, callbackRoute) {
  var customerData;
  let serviceName = payloadData.name;
  let secretKey;
  let apiKey;
  let serviceRecordId;
  async.series(
    [
      function (cb) {
        // Ensure that the account exists and isn't blocked
        // Ensure another app with the same name does not exist
        var query = {
          _id: userData.userId
        };
        var populate = {
          ...
          if (err) {
            ...
            if (data.length == 0) cb(ERROR.INCORRECT_ACCESSTOKEN);
              ...
              if (customerData.isBlocked) cb(ERROR.ACCOUNT_BLOCKED);
                ...
                if (serviceExists) cb(ERROR.SERVICE_NAME_EXISTS);
                else cb();
              }
            }
          }
      ...
      function (cb) {
        // Create an API key and create a new record in the serviceInfo collection
        apiKey = UniversalFunctions.generateApiKey();
        Service.ServiceInfoService.createRecord({
          apiKey: apiKey,
          name: serviceName,
          ...
          if (err) cb(err);
            ...
            serviceRecordId = data._id;
            cb();
          }
        });
      },
      function (cb) {
        // Set the secret key if it exists or generate one and push the API key record to the user's services array
        let updateData = {};
        if ('secretKey' in customerData) {
          secretKey = customerData.secretKey;
        } else {
          secretKey = UniversalFunctions.generateSecretKey();
          updateData.secretKey = secretKey;
        }

        updateData["$push"] = {
          services: serviceRecordId,
        };
